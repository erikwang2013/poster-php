...
├── native.php                  # native PHP entry: require it, no Composer needed
...
### Without Composer (native PHP)

Drop the whole `poster-php/` directory into your project and require `native.php`: it registers a PSR-4 autoloader and loads the global helpers, so no Composer and no framework are needed.

```php
require '/path/to/poster-php/native.php';   // autoloader + global helpers

$result  = captcha_create('click');
$builder = poster_create(750, 1334);
```

`native.php` is safe to include more than once and coexists with Composer or any other autoloader (prefer `vendor/autoload.php` when both are present).
...
// Returns: ['key' => '...', 'image' => 'data:image/png;base64,...',
//           'extra' => ['texts' => [['order' => 1, 'text' => '树'], ...]]]   // targets' coords stay server-side
...
$captcha = $manager->create('click')
    ->setDifficulty('hard')        // easy=2 targets | medium=3 | hard=4
    ->setTargetType('text')        // 'text' characters | 'icon' icons
