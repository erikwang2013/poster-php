...
├── native.php                  # entrada PHP nativa: basta un require, sin Composer
├── tests/                      # Pruebas PHPUnit, 53 archivos, con la misma estructura de directorios que src/
...
### Sin Composer (PHP nativo)

Coloca el directorio completo `poster-php/` dentro de tu proyecto e incluye `native.php` directamente: registra la autocarga PSR-4 y carga las funciones globales, sin necesidad de Composer ni de ningún framework.

```php
require '/path/to/poster-php/native.php';   // registra la autocarga + las funciones globales

$result  = captcha_create('click');
$builder = poster_create(750, 1334);
```

`native.php` se puede incluir varias veces y también convive con Composer o con el autocargador propio del proyecto (si se instala por duplicado, basta con dar prioridad a `vendor/autoload.php`).
...
`setTargetType('icon')` permite cambiar los textos objetivo por figuras vectoriales generadas programáticamente (11 tipos, dibujadas con primitivas de GD y sin necesidad de imágenes):
cada elemento de `extra['texts']` incluye un `thumb` adicional (la miniatura en base64 de esa figura) para que el frontend muestre la pista de clic; la verificación sigue siendo una comparación de coordenadas.
...
| Aleatoriedad | En cada generación varían el color de fondo, el ruido y las posiciones objetivo; además, cada objetivo de clic recibe tono y ángulo de rotación aleatorios |
| Límite por sesión | Límite de ventana que actúa entre keys (por defecto 30 veces en 60 segundos) y bloquea el intento a ciegas de «cambio de key y vuelvo a probar» |
| Trayectoria | Opcional (desactivada por defecto): valida el número de puntos, la duración y la linealidad de la trayectoria de arrastre, así que un script que envíe la respuesta por POST será rechazado |
...
| Tamaño mínimo del lienzo | Si el fondo es demasiado pequeño se lanza un error en lugar de degradar el resultado (el captcha de clic necesita al menos 120×120 y el de deslizador debe alojar una pieza de 4×2) |

#### Verificación de trayectoria (opcional)

Desactivada por defecto (para no perjudicar a las pantallas táctiles ni a los dispositivos de accesibilidad). Al activarla, `slider` / `rotate` requieren que el frontend envíe la trayectoria de arrastre y el servidor valida el número de puntos, la duración y la linealidad de la trayectoria:
...
'captcha' => [
    'trajectory' => [
        'enabled'      => true,
        'min_points'   => 4,      // puntos de muestreo mínimos
        'min_duration' => 300,    // duración mínima (milisegundos)
        'max_duration' => 5000,   // duración máxima (milisegundos)
        'max_linearity' => 0.99,  // por encima de esta linealidad se considera máquina (un arrastre por script es una línea recta)
    ],
],

// envío desde el frontend: la forma antigua con un número sigue siendo compatible
captcha_verify($key, 'slider', 173);
// con la verificación de trayectoria activada hay que enviar la trayectoria
...
$builder->save('/output/poster.jpg', 90);  // guardar en un archivo (ruta, calidad 0-100)
                                           // el formato se deduce de la extensión: jpg/jpeg/png/webp/gif
                                           // si no se pasa calidad, JPEG lee poster.jpeg_quality y PNG lee poster.png_compression
...
Cuando el contenido supera el límite de esa versión (por ejemplo, con nivel H, unos 1273 bytes o más) se lanza `InvalidArgumentException`, en lugar de generar en silencio un código que no se puede escanear.
...
```php
// tras poner captcha.route.enabled = true en config/poster.php, el adaptador registra un endpoint de imagen:
//   GET /captcha/{key} → devuelve el PNG directamente (Content-Type: image/png, Cache-Control: no-store)
// al frontend le basta la URL, sin tener que pasar base64 (un 33% menos de tamaño y cacheable por el navegador/CDN)
$result = Captcha::create('click')->generate();
// $result['image'] sigue siendo un data URI; $result['url'] es una dirección que se puede poner directamente en <img src>

// validación de formularios: el nombre de la regla es captcha y el parámetro es la key de la imagen
$request->validate([
